...
def mala_no_adapt(initial_value, pi, D_mala, sigma_2, lbda, n_steps):
    """
    No parameter is adapted in this version of the mala algorithm
    """
    values = np.zeros((n_steps + 1, len(initial_value)))
    values[0] = initial_value
    values_idx = 0
...
def mala_2(initial_value, pi, gamma, D_mala, mu_0, sigma_2_0, gamma_0, n_steps,
           epsilon_1, epsilon_2, A_1):
    values = np.zeros((n_steps + 1, len(initial_value)))
    values[0] = initial_value
    values_idx = 0
    ...
    for i in range(n_steps):
        ...
        # Step 1 : sample a candidate value
        mean = current_value + sigma_2_n / 2 * lambda_n.dot(D_mala(current_value))
        covariance = sigma_2_n * lambda_n
        candidate_value = np.random.multivariate_normal(mean, covariance)
        candidate_mean = (
            candidate_value
            + sigma_2_n / 2 * lambda_n.dot(D_mala(candidate_value))
        )
...
def mean_square_jump(values, n_steps):
    jumps = np.linalg.norm(values[1:] - values[:-1], axis=1)
    return np.sum(jumps) / n_steps
